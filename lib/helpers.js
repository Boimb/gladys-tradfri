...
module.exports = {
  ...
  createDeviceTypesFromDevice : (device) => {
    ...
    if (device instanceof Accessory && device.type === AccessoryTypes.remote) {
      ...
      sails.log.info(`Unhandled device detected : {name: ${device.name}, instanceId: ${device.instanceId}`)
...
const rgbBulbDeviceTypes = whiteBulbDeviceTypes.concat([ // RGB lights have at least white light deviceTypes
  // TODO check if hue and saturation are still props of Light and send by gateway on update
  {
    name: 'hue',
    type: 'multilevel',
    sensor: false,
